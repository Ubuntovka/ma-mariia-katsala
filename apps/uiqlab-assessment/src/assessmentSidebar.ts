	...
	* { box-sizing: border-box; }
	...
	<form id="assessment-form" novalidate>
		...
		<div class="hidden" id="deployment-url-fields">
			...
			<label class="share" id="share-row"><input id="share" type="checkbox" checked disabled><span>Allow this URL and the selected metrics to be sent to the evaluation service (required to run).</span></label></fieldset>
...
<script nonce="${nonce}">
	...
	function updateSource() { const deployedUrls = comparison() === 'deployment-urls'; const local = !deployedUrls && source() === 'local-url'; document.getElementById('url-label').textContent = local ? 'Local URL' : 'Deployment URL'; url.placeholder = local ? 'http://localhost:3000' : 'https://example.com'; document.getElementById('share-row').style.display = local ? 'none' : 'flex'; document.querySelector('#share-row span').textContent = deployedUrls ? 'Allow both URLs and the selected metrics to be sent to the evaluation service (required to run).' : 'Allow this URL and the selected metrics to be sent to the evaluation service (required to run).'; updateHistoricalSelectors(); save(); }
